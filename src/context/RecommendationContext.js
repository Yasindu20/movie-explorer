...
export const RecommendationProvider = ({ children }) => {
  ...
  const addToWatchHistory = useCallback(async (movieId) => {
    try {
      ...
      if (existingMovie) {
        ...
        // Fetch full movie details and add to history
  ...
  const getPersonalizedTopPicks = useCallback(async () => {
    try {
      dispatch({ type: actions.SET_LOADING, payload: true });
      
      // Get recommendations from different sources if not already loaded
      const historyRecs = state.recommendedMovies.basedOnHistory.length > 0 
        ? state.recommendedMovies.basedOnHistory 
        : await getRecommendationsBasedOnWatchHistory();
        
      const genreRecs = state.recommendedMovies.basedOnGenres.length > 0
        ? state.recommendedMovies.basedOnGenres
        : await getRecommendationsBasedOnGenres();
        
      const actorRecs = state.recommendedMovies.basedOnActors.length > 0
        ? state.recommendedMovies.basedOnActors
        : await getRecommendationsBasedOnActors();
